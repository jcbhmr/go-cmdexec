...
This module also offers `ExecProcess` (`os.StartProcess` equivalent) for lower-level process execution.
...
- `ExecProcess` is implemented in terms of `execProcess`.
- `execProcess` is platform-specific.
- `execProcess` on `unix` is delegates to `execProcessUnix`.
- `execProcess` on `unix` is based on [`syscall.forkExec`](https://github.com/golang/go/blob/go1.25.6/src/syscall/exec_unix.go#L143).
- `execProcess` on `plan9` doesn't need to delegate further.
- `execProcess` on `plan9` is based on [`syscall.forkExec`](https://github.com/golang/go/blob/go1.25.6/src/syscall/exec_plan9.go#L362).
- `execProcessUnix` has platform-specific implementations for various Unix platforms.
- `execProcessUnix` implementations are based on platform-specific `syscall.forkAndExecInChild` implementations.
